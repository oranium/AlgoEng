...
Matrix2D gaussFilter(ppm& img, int size, double sig){
    ...
    return ppmConvolveBW(gauss_filter,  img);
}
...
Matrix2D sigmaFilter(Matrix2D& img, Matrix2D& blurred_image){
    ...
    for(auto ele: img){
        ...
        double gamma = 0.5 +(min_val/ max_value) * min_val * min_val / 65025;
...
Matrix2D meanFilter(ppm& img, int size){
    ...
    return ppmConvolveBW(mean_filter,  img);
}
